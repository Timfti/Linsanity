...
/**
 * Servlet implementation class BannersServlet
 */
@WebServlet("/BannersServlet")
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		switch(action)
		{
			case "edit":		
				...
				String BannerUrl1 = request.getParameter("BannerUrl");


				if(Id>0) {
					Banners model = server1.getById(Id);
					model.setBannerUrl(BannerUrl1);
			...
			case "get":
				...
				Banners obj = server1.getById(Id3);
				
				String jsonString = "";
				jsonString += "{";
				jsonString += "\"Id\":"+ obj.getId();
				jsonString += ",\"BannerUrl\":\"" + obj.getBannerUrl() + "\"";
				...
				break;
			case "getList":
				List<Banners> list = server1.queryAll();
				if(list !=null && list.size()>0) {
					result.set_ok(true);
					List<String> json= new ArrayList();
					for(int i = 0; i < list.size(); i++) {
						Banners obj2 = list.get(i);
						String jsonString2 = "";

						jsonString2 += "{";
						jsonString2 += "\"Id\":"+ obj2.getId();
						jsonString2 += ",\"BannerUrl\":\"" + obj2.getBannerUrl() + "\"";
			...
			case "getUserList":
				...
				List<Banners> list2 = server1.queryUserAll(userId);
				if(list2 !=null && list2.size()>0) {
					result.set_ok(true);
					List<String> json2= new ArrayList();
					for(int i = 0; i < list2.size(); i++) {
						Banners obj2 = list2.get(i);
						String jsonString3 = "";

						jsonString3 += "{";
						jsonString3 += "\"Id\":"+ obj2.getId();
						jsonString3 += ",\"BannerUrl\":\"" + obj2.getBannerUrl() + "\"";
