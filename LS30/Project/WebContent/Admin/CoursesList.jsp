<%@ page language="java" contentType="text/html; charset=utf-8" pageEncoding="utf-8"%>
	...
	<a href="CoursesEdit.jsp" class="link-btn">添加</a>
		...
		$.get("/Project/CoursesServlet?action=getList","",function(data){ 
			...
			if(result.IsOK){
				for(var i=0; i < result.Data.length; i++)
				{
					...
					html += '<td>'+obj.Id +'</td>';
					html += '<td>'+obj.Name +'</td>';
					html += '<td><img src="/Project/'+obj.FilePath +'" style="height:50px;" /></td>';
					html += '<td>'+obj.TimeLength +'</td>';
					html += '<td>'+obj.Remark +'</td>';

					html += '<td>'; 
					html += '<a href="CoursesEdit.jsp?Id='+obj.Id+'">编辑</a>'; 
					html += '<div style="display:inline-block; margin-left:15px;" onclick="del('+obj.Id+');">删除</div>';		
					html += '<a href="CourseChaptersList.jsp?cId='+obj.Id+'" style="display:inline-block; margin-left:15px;">章节管理</a>'; 
					html += '</td>'; 
					html += '</tr>';
				}
			}
	...
	var del = function(Id){
		$.get("/Project/CoursesServlet?action=delete&Id="+Id, function(data){
			var result = JSON.parse(data);
			if(result.IsOK){
				alert("删除成功");
